...
//	printf(">>>LoadData, \n");
//	printf("ch[0~3]:%03d,%03d,%03d,%03d, s_l:%d, s_r:%d\n", RC_Key_V3A.ch[0], RC_Key_V3A.ch[1], RC_Key_V3A.ch[2], RC_Key_V3A.ch[3], RC_Key_V3A.s_l, RC_Key_V3A.s_r);
...
void AgreementOut_RunHandle(void)
{
	...
	{
			memset(&RC_Key_V3A, 0, sizeof(RC_Key_V3A));
			if(AgreementOut_OutStrToUart() == HAL_OK)
				isOffline = true;
	}
}
