...
HAL_StatusTypeDef Info_Nrf_Slave_Init(uint8_t channel)
{
	...
	Info_Nrf_write_reg( NRF_WRITE_REG + SETUP_RETR, 0x4F);
...
//已测试非常正确
HAL_StatusTypeDef Info_Nrf_ChangeModeTo(LimeNrfMaster_BspMode_e mode)
{
	Info_Nrf_CE_Ctrl(0);
...
{
	while(INFO_NRF_IRQ_LEVEL_READ() == GPIO_PIN_RESET)
	{
		Info_Nrf_write_reg(FLUSH_TX, 0xff);
		LEprintf("errrrrr\n");
	}
	...
	{
		LimeInfoNrf.lastSuccRunTime = HAL_GetTick();
		return HAL_OK;
	}
