...
{
	HAL_SPI_TransmitReceive(&hspi4, txData, rxDate, length, 1000);
}
...
static void printErrorCode(uint8_t errID)
{
	...
	{
		case 1:
			LEprintf("Err:1, LostPack\n");
			break;
		case 2:
			LEprintf("Err:2, More Pack\n");
			break;
		default:break;
	}
}
...
uint8_t FastNrf_NrfIsOnline(void)
{
  ...
  uint8_t i;
	...
	if(HAL_GPIO_ReadPin(NRF1_IRQ_GPIO_Port, NRF1_IRQ_Pin) == GPIO_PIN_RESET)
	{
...
//		offset += 32;
//		FastNrf_writeBuf(WR_TX_PLOAD, data + offset, 32);
//		break;
//	}
}
...
//	while(offset < length)
//	{
//		if(configInfo->recvFinishFlag)
//		{
//			configInfo->recvFinishFlag = 0;
//			
//			FastNrf_read_buf(RD_RX_PLOAD, data + offset, 32);
//			offset += 32;
//		}
//	}
}
...
int8_t FastNrf_InitTemplete(NRF_Mode_e mode)
{
	if(mode == NRF_Mode_Tx)
	{
		if(FastNrf_NrfIsOnline())
		{
			LEprintf("Nrf Online, Mode:Tx\n");
...
void LEprintfBuf(uint8_t* buf, uint32_t length)
{
	LEprintf("Buf:\n");
